from Week_8_init import *
import traceback, sys
#
# try out asyncio for the radio 
import asyncio
...
async def dispatch_2 ():
    ...
    while (1):
        ...
        if len(rq):       # if there's nothing in the receive queue, exit
            ...
            try:
                cmd = packet_text[0:6].decode('ascii')
            except Exception as e:
                print ("ASCII decode in dispatch_2")
                print (e)
                print(traceback.format_exception(e))
                continue
            if (packet_text[0:6].decode('ascii') == "BEACON"):
                    print("\nReceived a beacon: {}".format(packet_text))
                    continue
            if (len(snaq) >= 1 and packet_text[0:3].decode('ascii') == "ACK" and len(packet_text) >=7): 
                    # rewrite snaq queue without this acksn
                    print("\nACK received for packet {}".format(packet_text.decode('ascii')))
            ...
            try:
                packet_text = packet_text[5:]      #  Strip off the acksn
                if (packet_text[0:9].decode('ascii') == "TELEMETRY"):
                    ...
                    exec_cmd_cnt += 1
                    speed = float(packet_text[5:10].decode('ascii'))
                    mq.append(speed/10.0)       #  queue up the motor move
                elif (packet_text[0:2] == "HK"):
                    exec_cmd_cnt += 1
                    tq.append(bytes("{} TELEMETRY: snaq={}; tq={}; rq={}; mq={}; bad={}; exec={}".format(acksn, len(snaq), len(tq), len(rq), len(mq), bad_cmd_cnt, exec_cmd_cnt), "utf-8"))
                    snaq.append((bytes("{} TELEMETRY: snaq={}; tq={}; rq={}; mq={}; bad={}; exec={}".format(acksn, len(snaq), len(tq), len(rq), len(mq), bad_cmd_cnt, exec_cmd_cnt), "utf-8")))
                    acksn += 1
...
async def beacon_2():
    ...
    while (1):
        ...
        print("\nSent our BEACON: snaq={}; tq={}; rq={}; mq={}; bad={}; exec={}".format(len(snaq), len(tq), len(rq), len(mq), bad_cmd_cnt, exec_cmd_cnt))
...
async def transmit_2 (radio_lock):
    ...
    global rfm69
    while (1):
        print ("t{}".format(len(tq)), end="")
        await asyncio.sleep(0)
    #  see if there's anything to deque and deque it
        if (len(tq) >= 1):
            cmd = tq.popleft()
            print ("c{}".format(len(tq)), end="")
            try:
            ...
            #  transmit it
                    ...
                    rfm69.idle()
                    print ("s", end="")
                    rfm69.send(cmd) #  Comment out to make a monitoring ground station
                    print ("T{}".format(len(tq)), end="")
...
# enque a transmission rfm69.send(cmd) with asksn and ask for confirmation
            tq.append(("{:4d}" + cmd).format(acksn))
            snaq.append(("{:4d}" + cmd).format(acksn))
            acksn += 1
...
async def main():
    ...
    if SAT:
        ...
        task1 = asyncio.create_task(factorial("job1fact", 3))
        ...
        await asyncio.gather( task1, task2, task3, task4, task5, task6, task8, task9, task10)
        ...
        task9 = asyncio.create_task(resend_2())
        task10 = asyncio.create_task(housekeeping_2())
        await asyncio.gather( task1, task2, task3, task4, task6, task7, task8, task9, task10)
        print(f"Should never get here")
